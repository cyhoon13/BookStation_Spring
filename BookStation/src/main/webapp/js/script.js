...
$(document).ready(function() {
    // 첫 번째 체크박스(전체 선택) 선택자
    var $checkAll = $('.table-shopping-cart .table_head input[type="checkbox"]');
    
    // 나머지 체크박스(개별 선택) 선택자
    var $checkboxes = $('.table-shopping-cart .table_row input[type="checkbox"]');

    // 전체 선택 체크박스의 변경 이벤트 리스너
    $checkAll.change(function() {
        var isChecked = $(this).is(':checked'); // 전체 선택 체크박스의 상태
        $checkboxes.prop('checked', isChecked); // 모든 개별 체크박스의 상태 변경
    });

    // 개별 체크박스의 변경 이벤트 리스너 (전체 선택 상태 업데이트)
