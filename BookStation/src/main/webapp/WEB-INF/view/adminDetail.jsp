         ...
         <div class="row">
            ...
            <div class="col-md-8 col-lg-9 p-b-80 order-md-2">
               <div class="p-r-45 p-r-0-lg">
                  <!-- 비밀번호 -->
                  <form action="updatePassword2.do" method="post">
                     <div class="con">
                        <div class="title">
                           <h3>회원정보 조회</h3>
...
<!--===============================================================================================-->
   <script>
      ...
      // 비밀번호와 비밀번호 확인에 비밀번호가 같은지 확인
      $(document).ready(function() {
          $('#passwordChangeButton').on('click', function(event) {
              var newPassword = $('#newPwd').val();
              var newPasswordCheck = $('#newPwdCheck').val();
              var passwordPattern = /^(?=.*[a-z])(?=.*[0-9])[A-Za-z0-9!@#$%^&*?=+_-]{8,16}$/;

              if (newPassword !== newPasswordCheck) {
                  alert('새 비밀번호가 일치하지 않습니다.');
                  event.preventDefault(); // 폼 제출을 막음
              } else if (!passwordPattern.test(newPassword)) { //새 비밀번호가 정규 표현식 패턴을 따르지 않으면 경고 메시지를 출력
                  alert('비밀번호는 영문과 숫자를 포함한 8~16자입니다.');
                  event.preventDefault(); // 폼 제출을 막음
              } else {
